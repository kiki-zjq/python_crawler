...
class ZhihuSpider(Spider):
    ...
    # 关注链接
    follow_url ='https://www.zhihu.com/api/v4/members/{user}/followees?include=data&offset={offset}&limit=10'

    # 粉丝链接
    fan_url = 'https://www.zhihu.com/api/v4/members/{user}/followers?include=data&offset={offset}&limit=10'

    start_users = ['evanyou','liaoxuefeng','wan-lai-tian-yu-xue-29-52']
    #start_users = ['miloyip']
    ...
    def parse_user(self,response):
        ...
        if result:
            ...
            user_item = UserItem()
            field_map = {
                'id':'id','user': 'url_token', 'name': 'name', 'headline': 'headline', 'description': 'description',
        ...
        if result.get('data'):
            ...
            for data in datas:
                ...
                if question:
                    ...
                    answer_map = {
                        'id':'id','answer_text':'excerpt','voteup_count':'voteup_count','comment_count':'comment_count',
                        'is_copyable':'is_copyable','answer_created':'created_time','answer_updated':'updated_time'
                    }
                    for field, attr in answer_map.items():
                        answer_item[field] = data.get(attr)
